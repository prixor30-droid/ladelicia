...
# Empaque (bolsas/fundas en kg) — mismo motivo, a nivel módulo.
EMPAQUES_INFO = [
    ("Transparente",       "📦", "kg", "emp"),
    ("BBQ emp",            "📦", "kg", "emp"),
    ("Limón emp",          "📦", "kg", "emp"),
...
def calcular_caja_real_hoy(recibido_fab=False, recibido_carro=False):
    ...
    session_state — si todavía no se han tocado hoy, se asume que no).
    """
    fecha_cr = fecha_hoy()
    ...
    caja_real_cr = (
        ...
        + (total_fab_cr if recibido_fab else 0)
        + (total_carro_cr if recibido_carro else 0)
        - total_egresos_cr
    )

    return {
        "total_fab": total_fab_cr, "total_carro": total_carro_cr,
        "efectivo_inicial": efectivo_inicial_cr, "ingresos_manuales": ingresos_manuales_cr,
        "total_egresos": total_egresos_cr, "caja_real": caja_real_cr,
    }
...
def _registrar_auditoria_stock(tabla_origen, registro_id, accion, detalle, usuario):
    """Deja constancia de quién editó o eliminó un registro de Producción o Cargues
    ya existente — estas grillas antes sobrescribían/borraban sin dejar rastro."""
    sb_post("auditoria_stock", {
        "fecha": fecha_hoy(), "hora": ahora(),
        "tabla_origen": tabla_origen, "registro_id": registro_id, "accion": accion,
        "detalle": detalle, "usuario": usuario or "Sin especificar"
    })
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    quien_ajusta = st.selectbox("¿Quién hace este ajuste?", EMPLEADOS, key="quien_ajusta")
    if st.button("💾 Guardar ajuste", key="btn_adj"):
        if nuevo_stock != stock_adj:
            sb_post("ajustes_stock_manual", {
                ...
                "delta": nuevo_stock - stock_adj, "usuario": quien_ajusta
            })
    ...
    st.markdown('<div class="section-label">🗑️ Registrar producto dañado</div>', unsafe_allow_html=True)
    st.caption("Para cuando algo se daña o se pierde después de producido — solo dices cuánto, la app resta el stock sola (no hay que calcular el número final a mano).")
    sabor_dan = st.selectbox("Sabor", SABORES_LISTA, key="sabor_dan")
    stock_dan_disp = get_stock(sabor_dan)
    cant_dan = st.number_input("Cantidad dañada", min_value=1, max_value=max(1, stock_dan_disp), value=1, step=1, key="cant_dan")
    motivo_dan = st.text_input("Motivo (opcional)", key="motivo_dan", placeholder="Ej: se rompió el empaque, quedó rancio")
    quien_dana = st.selectbox("¿Quién registra este daño?", EMPLEADOS, key="quien_dana")
    if st.button("🗑️ Registrar daño", key="btn_dan"):
        sb_post("mermas_stock", {
            "fecha": fecha_hoy(), "hora": ahora(), "sabor": sabor_dan,
            "cantidad": cant_dan, "motivo": motivo_dan.strip(), "usuario": quien_dana
        })
        restar_stock(sabor_dan, cant_dan)
        st.session_state.ok_dan = True
        time.sleep(0.3)
        st.rerun()
    ...
    with tab_caja2:
        ...
        with st.expander("📋 Ver últimos movimientos de hoy"):
            ...
            if ult_movs:
                for m in ult_movs:
                    st.markdown(
                        f'<div class="factura-row"><span>{m["tipo"]} {m["hora"]} — {m["concepto"]}</span><span><b>{fmt(m["valor"])}</b></span></div>',
                        unsafe_allow_html=True
                    )
                st.caption("Solo lectura — para borrar un movimiento mal registrado ve a 📋 Historial.")
            else:
...
# bono semestral, historial de pagos. Cada pago registrado también se guarda
# como egreso en caja_egresos (categoría "Salario") para que siga contando en
# el arqueo de caja y en el reporte de Contador → Historial por empleado.
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.get("ok_nomina"):
        ...
        st.session_state.ok_nomina = False

    # ── Empleados ──
    with sub_n1:
        st.markdown('<div class="section-label">Empleados activos</div>', unsafe_allow_html=True)
        raw_emp_n1 = sb_get("nomina_empleados", "select=*&activo=eq.true&order=nombre.asc") or []
        if raw_emp_n1:
            df_emp_n1 = pd.DataFrame([{
                "Nombre": r["nombre"], "Salario mensual": fmt(r["salario_mensual"]),
                "Fecha ingreso": r["fecha_ingreso"],
                "Tipo": "Variable" if r["tipo"] == "variable" else "Fijo",
                "Bono semestral": "Sí" if r.get("bono_semestral") else "No",
            } for r in raw_emp_n1])
            tabla_view(df_emp_n1)
        else:
        ...
        if st.button("✅ Agregar empleado", key="btn_add_ne"):
            if not nombre_ne.strip():
                ...
                st.markdown(f'<div class="alert-low">{ICO_WARN} Ingresa el salario mensual.</div>', unsafe_allow_html=True)
            else:
        ...
        if raw_emp_n1:
            st.markdown('<div class="section-label">Editar salario</div>', unsafe_allow_html=True)
            nombres_edit_ne = [r["nombre"] for r in raw_emp_n1]
            emp_edit_sel_ne = st.selectbox("Empleado", nombres_edit_ne, key="emp_edit_sel_ne")
            emp_edit_obj_ne = next(r for r in raw_emp_n1 if r["nombre"] == emp_edit_sel_ne)
            nuevo_salario_ne = st.number_input(
                "Nuevo salario mensual ($)", min_value=0,
                value=int(emp_edit_obj_ne["salario_mensual"]), step=50000, key="nuevo_salario_ne"
            )
            if st.button("💾 Actualizar salario", key="btn_edit_salario_ne"):
                sb_patch("nomina_empleados", f"id=eq.{emp_edit_obj_ne['id']}", {"salario_mensual": float(nuevo_salario_ne)})
                st.markdown(f'<div class="success-toast">{ICO_CHECK} Salario actualizado.</div>', unsafe_allow_html=True)
                time.sleep(0.3); st.rerun()
    ...
    # ── Ausencias ──
    with sub_n2:
        st.markdown('<div class="section-label">Registrar día no trabajado</div>', unsafe_allow_html=True)
        raw_emp_n2 = sb_get("nomina_empleados", "select=id,nombre&activo=eq.true&tipo=eq.fijo&order=nombre.asc") or []
        if not raw_emp_n2:
            ...
            if st.button("✅ Registrar ausencia", key="btn_aus"):
                if not motivo_aus.strip():
                    st.markdown(f'<div class="alert-low">{ICO_WARN} Escribe el motivo.</div>', unsafe_allow_html=True)
                else:
                    id_emp_aus = next(r["id"] for r in raw_emp_n2 if r["nombre"] == emp_aus_sel)
                    sb_post("nomina_ausencias", {"empleado_id": id_emp_aus, "fecha": str(fecha_aus), "motivo": motivo_aus.strip()})
                    st.markdown(f'<div class="success-toast">{ICO_CHECK} Ausencia registrada.</div>', unsafe_allow_html=True)
                    time.sleep(0.3); st.rerun()

        st.markdown('<div class="section-label">Ausencias de este mes</div>', unsafe_allow_html=True)
        primer_dia_aus = datetime.now(COL_TZ).date().replace(day=1)
        hoy_aus = datetime.now(COL_TZ).date()
        raw_aus_mes = sb_get("nomina_ausencias", f"select=id,empleado_id,fecha,motivo&fecha=gte.{primer_dia_aus}&fecha=lte.{hoy_aus}&order=fecha.desc") or []
        if raw_aus_mes:
            raw_emp_todos_n2 = sb_get("nomina_empleados", "select=id,nombre") or []
            nombre_por_id_aus = {r["id"]: r["nombre"] for r in raw_emp_todos_n2}
            df_aus = pd.DataFrame([{
    ...
    with sub_n3:
        ...
        if fecha_ref_n.day <= 15:
            ...
            periodo_ini_n = date(fecha_ref_n.year, fecha_ref_n.month, 16)
            if fecha_ref_n.month == 12:
    ...
    with sub_n4:
        ...
        emp_hist_sel = st.selectbox("Empleado", nombres_hist_n4, key="emp_hist_sel")

        raw_pagos_n4 = sb_get("nomina_pagos", "select=*&order=fecha_pago.desc,periodo_inicio.desc") or []
        if emp_hist_sel != "Todos":
            id_filtro_n4 = next((eid for eid, n in nombre_por_id_n4.items() if n == emp_hist_sel), None)
            raw_pagos_n4 = [r for r in raw_pagos_n4 if r["empleado_id"] == id_filtro_n4]

        if not raw_pagos_n4:
            st.info("Sin pagos registrados todavía.")
        else:
            total_hist_n4 = sum(float(r["total_pagado"]) for r in raw_pagos_n4)
            st.markdown(f'<div class="calc-box">💵 Total pagado: <b>{fmt(total_hist_n4)}</b> ({len(raw_pagos_n4)} pagos)</div>', unsafe_allow_html=True)
